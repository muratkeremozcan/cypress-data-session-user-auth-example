...
describe('command tests - test the lego blocks that will build our uber function', () => {
  ...
  it('cy.createUserWithToken() should create a user with a token', () => {
    cy.createUserWithToken('someSuperToken', admin).should((user) => {
      expect(user.email).to.equal('auth-admin@gmail.com')
      expect(user.accessToken).to.equal(
        'token-for-auth-admin@gmail.com-p@22w0rd'
      )
    })
  })
